...
def fast_test_p_s(model, all_train_sets, all_test_only_sets,set_index, args, logger=None,writer=None, return_avg=False):
    # using Cython test during train
    # return mAP, Rank-1
    print(f'****** start perform fast testing! ******')
    ...
    for temp_loaders in loaders:
        ...
        with torch.no_grad():
            for i, (imgs, _,fnames, pids, cids, domains) in enumerate(test_loader):
    ...
    if writer is not None:
            ...
            writer.add_scalar(tag="results/UnSeen-Avg_R@1", scalar_value=aver_unseen_r1,
                              global_step=set_index)
    if return_avg:
        return aver_seen_mAP,aver_seen_r1
    return map
